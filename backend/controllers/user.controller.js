...
const getSuggestUsers = async (req, res) => {
  try {
    ...
    const suggestedUsers = filteredUser.slice(0, 2);
    suggestedUsers.forEach((user) => (user.password = null));

    res.status(300).json(suggestedUsers);
  } catch (error) {
...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    if (profileImg) {
      if (user.profileImg) {
        //res.cloudinary.com/dhz4w3dmx/image/upload/v1732435520/cld-sample-5.jpg
...
export { getUserProfile, followUnfollowUser, getSuggestUsers, updateUser };
